...
import { assignedInterns, mentorIncomingRequests, mentorTasks, mentorConversations } from '../../utils/mockData';

export default function MentorDashboard() {
  const { user } = useAuth();
  const navigate = useNavigate();

  const pendingRequests = mentorIncomingRequests.filter((r) => r.status === 'pending').length;
  const activeTasks = mentorTasks.filter((t) => t.status === 'in_progress' || t.status === 'pending').length;
  const unreadMessages = mentorConversations.reduce((sum, c) => sum + c.unread, 0);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <section>
          ...
          <div className="space-y-3">
            {mentorIncomingRequests.slice(0, 2).map((request) => (
              <IncomingRequestCard key={request.id} request={request} onAccept={() => {}} onDecline={() => {}} />
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3">
            {assignedInterns.filter((i) => i.status === 'active').slice(0, 2).map((intern) => (
      ...
      <section className="mt-8">
        ...
        <div className="space-y-3">
          {mentorTasks.filter((t) => t.status !== 'completed').slice(0, 3).map((task) => (
            <MentorTaskCard key={task.id} task={task} onEdit={() => {}} onDelete={() => {}} />
          ))}
        </div>
      </section>
